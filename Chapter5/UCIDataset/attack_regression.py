...
adv = analyzers.FGSM(bayes_model, np.asarray(X_test), eps=EPSILON, loss_fn=loss)
y_pred = bayes_model.predict(adv, n=35)
check_rmse = tf.keras.metrics.RootMeanSquaredError(name="eval_rmse")
check_rmse(y_test, y_pred)
print("Adversarial RMSE (FGSM): ", check_rmse.result())


adv = analyzers.PGD(bayes_model, np.asarray(X_test), eps=EPSILON, loss_fn=loss, num_steps=10, restarts=2)
y_pred = bayes_model.predict(adv, n=35)
check_rmse = tf.keras.metrics.RootMeanSquaredError(name="eval_rmse")
check_rmse(y_test, y_pred)
print("Adversarial RMSE: ", check_rmse.result())
...
attacks.append(adv)
p_upper, _ = IBP_upper(bayes_model, img, img, MARGIN, SAMPLES, loss_fn=loss, eps=EPSILON, predicate=predicate_unsafe, inputs=attacks, inflate=2.0, mod_option=10)
p_upper = 1 - p_upper
